...
flowchart TD
    ...
    G --> H["创建 ConversionOptions"]
    H --> I["ConversionWorker 调用 Converter.convert"]
    I --> J["FFmpeg 转码和提取封面"]
    J --> K["Apple、Android、vivo 分别封装"]
    K --> L["反向解析校验"]
...
保存 input_path、output_dir、start_time、duration、cover_time、mute 和 quality。
...
集中保存 Apple、Android、vivo、Windows、manifest 和说明文件路径。files 属性将它们组成 tuple，调用方不必重复写清单。
...
schema_version 当前为 2，记录生成时间、源文件、配对 ID、用户参数、原视频信息以及每个输出的角色、名称、大小和哈希。
...
片段开始和封面都有数值框与滑块。程序使用 blockSignals 防止双方更新时形成循环。
...
ConversionWorker 保存 threading.Event：
...
转换中禁用输入、滑块、画质和目录选择，只允许取消，避免界面参数与后台实际参数不一致。
...
~~~powershell
python -m livephoto convert ".\input.mp4" --output ".\output" --start 2.5 --duration 3 --cover 3.8 --quality high
~~~
